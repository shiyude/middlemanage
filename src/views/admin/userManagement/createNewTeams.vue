    ...
    <div class="head">
      <el-row>
        ...
        <el-col :span="12" style="text-align:right">
          <el-button size="mini" @click="addTeam" :disabled="tableDataInit!==-1">{{$t('createNewTeam.addTeam')}}</el-button>
          <el-button size="mini" @click="refresh">{{$t('createNewTeam.refresh')}}</el-button>
        </el-col>
      </el-row>
    </div>
    <div class="teams_content">
      <el-table :data="teamList" border style="width: 100%" :header-cell-style="{'background':'#f5f7fa'}" size="mini">
        ...
        <el-table-column :label="$t('teams.sellingEntity')">
          <template slot-scope="scope">
            <el-input v-if="scope.$index === tableDataInit" v-model.trim="sellingEntities"></el-input>
        ...
        <el-table-column :label="$t('teams.manageMembership')">
          <template slot-scope="scope">
            <el-button size="mini" plain @click="manageMembership(scope.row)" v-if="scope.row.teamId">{{$t('teams.manageMembership')}}</el-button>
          </template>
        </el-table-column>
        <el-table-column :label="$t('teams.edit')">
          <template slot-scope="scope">
            ...
            <template v-else>
              <el-button size="mini" plain @click="edit(scope.row,scope.$index)">{{$t('teams.edit')}}</el-button>
            </template>
          </template>
        </el-table-column>
        <el-table-column :label="$t('teams.delete')">
          <template slot-scope="scope">
            <el-button size="mini" plain @click="deleteData(scope.row)" :disabled="scope.$index === tableDataInit">{{$t('teams.delete')}}</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="page_section" v-if="teamCount">
        <el-pagination background small @size-change="handleTeamSizeChange" @current-change="handleTeamCurrentChange" :current-page.sync="teamCurrentPage" :page-sizes="[5,10,30,50,100]" :page-size="teamPageSize" layout="prev, pager, next,sizes,total" :total="teamCount">
        </el-pagination>
      </div>
    </div>
    <!-- Manager Membership -->
    <el-dialog :title="$t('teams.manageMembership')" :visible.sync="dialogVisible" width="900px" class="new_team_dialog" @close="closeDialog">
      <el-row class="row_header">
        <el-col :span="24" class="col_button">
          ...
          <uploader ref="newTeamFile" fileId="newTeamFile" :maxSize="10" :uploadParam="uploadParam" @uploadAfter="uploadAfter" :url="$api.importTeamAgent" fileType=".xls,.xlsx" :btnText="{select:$t('createNewTeam.SelectFile'),import:$t('createNewTeam.Import')}"></uploader>
          <el-button size="mini" @click="exportNewTeam">{{$t('createNewTeam.Export')}}
          </el-button>
        ...
        <el-col :span="17">
          <el-select v-model="selectValue" size="mini" style="width:100%" filterable>
            ...
            <div class="el_select_div">
              <el-option v-for="item in selectLists" :key="item.agentId" :label="item.agentName" :value="item.agentId" class="table_option">
      ...
      <el-table :data="dialogList" border style="width: 100%" :header-cell-style="{'background':'#f5f7fa'}" size="mini">
        ...
        <el-table-column prop="regNum" :label="$t('teams.regNum')">
        </el-table-column>
        <el-table-column :label="$t('teams.delete')">
          <template slot-scope="scope">
      ...
      <div class="page_section" v-if="dialogCount">
        <el-pagination background small @size-change="handleDialogSizeChange" @current-change="handleDialogCurrentChange" :current-page.sync="dialogCurrentPage" :page-sizes="[5,10,30,50,100]" :page-size="dialogPageSize" layout="prev, pager, next,sizes,total" :total="dialogCount">
        </el-pagination>
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    exportNewTeam() {//导出team成员
      window.location.href = this.$addDownUrl(this.$api.exportTeamAgent, {
        teamId: this.teamId
      });
    },
    uploadAfter() {//文件上传完后的回调
      this.getDialogLists();
      this.getSelectData();
    },
    deleteAllMember() {    //删除team的所有成员
    ...
    manageMembership(row) {  //点击manageMembership按钮
    ...
    handleDialogSizeChange(val) {//弹窗内的分页
      this.dialogPageSize = val;
      this.getDialogLists();
    },
    handleDialogCurrentChange(val) {   //弹窗内的分页
      this.dialogCurrentPage = val;
      this.getDialogLists();
    },
    handleTeamSizeChange(val) {      //team分页
      this.teamPageSize = val;
      this.getTeamLists();
    },
    handleTeamCurrentChange(val) {      //team分页
