  ...
  <div class="user_list_wrapper">
    <div class="user_list user_section">
      <el-row class="row_header">
        <el-col :span="6" class="col_text">
          {{$t('userLists.userList')}}
        </el-col>
        <el-col :span="18" class="col_button">
          <uploader fileId="userFile" :maxSize="100" :uploadParam="uploadParam" @uploadAfter="uploadUserAfter" :url="$api.importUsers" fileType=".csv" :btnText="{select:$t('userLists.selectFile'),import:$t('userLists.createMultipleAccounts')}"></uploader>
          <el-button size="mini" @click="downloadTemplate"> {{$t('userLists.downloadTemplate')}}
          </el-button>
        </el-col>
      </el-row>
    </div>
    <div class="user_list_table user_section">
      ...
      <el-table :data="userList" border style="width: 100%" :header-cell-style="{'background':'#f5f7fa'}" size="mini">
        ...
        <el-table-column :label="$t('userLists.password')">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('userLists.expirationDate')">
          <template slot-scope="scope">
            <el-date-picker type="date" value-format="timestamp" v-if="scope.$index === tableDataInit" v-model.trim="expirationDate">
            </el-date-picker>
            <div v-else>{{$dateFormatNoTime(scope.row.expirationDate)}}</div>
          </template>
        </el-table-column>
          <el-table-column :label="$t('Lastlogontime')">
          <template slot-scope="scope">
            <div>{{$dateFormatNoTime(scope.row.lastUpdateTime)}}</div>
          </template>
        </el-table-column>
