  ...
  <div class="plan_wrapper">
    <div class="stack_plans plans_section">
      <el-row class="row_header">
        ...
        <el-col :span="16" class="col_button">
          <uploader fileId="planFile" :maxSize="50" :uploadParam="uploadParam" @uploadAfter="uploadPlanAfter" :url="$api.uploadSitePlan" fileType=".png,.jpg,.zip" :btnText="{select:$t('plan.selectFile'),import:$t('plan.addPlanImage')}"></uploader>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="15">
          <el-alert :title="$t('plan.addPlanImageCommand')" type="info" :closable="false">
          </el-alert>
        </el-col>
      </el-row>
    </div>
    <div class="create_plan plans_section">
      ...
      <div class="el-btn-div">
        <el-button size="mini" @click="createSitePlan('siteplan')">{{$t('plan.createSitePlan')}}</el-button>
        <el-button size="mini" @click="createSitePlan('mobilechart')">{{$t('plan.createMobileChart')}}
        </el-button>
        <el-button size="mini" @click="createSitePlan('chart')">{{$t('plan.createChart')}}
        </el-button>
      </div>
    ...
    <div class="plan_lists plans_section">
      <el-row class="row_header">
        ...
        <el-col :span="12" class="col_button">
          <el-button size="mini" @click="refresh">{{$t('plan.refresh')}}
          </el-button>
        </el-col>
      </el-row>
      <el-table :data="planList" border style="width: 100%" :header-cell-style="{'background':'#f5f7fa'}" size="mini">
        ...
        <el-table-column :label="$t('plan.showinMobile')">
          <template slot-scope="scope">
            <el-checkbox v-model="showInMobile" v-if="scope.$index === tableDataInit" :true-label="'YES'" :false-label="'NO'"></el-checkbox>
            <div v-else>
        ...
        <el-table-column :label="$t('plan.showInterest')">
          <template slot-scope="scope">
            <el-checkbox v-model="showInterest" v-if="scope.$index === tableDataInit" :true-label="'yes'" :false-label="'no'"></el-checkbox>
            <div v-else>
        ...
        <el-table-column :label="$t('plan.editMapping')">
          <template slot-scope="scope">
            <el-button size="mini" v-if="scope.row.type!=='table'" plain :disabled="scope.$index === tableDataInit" @click="toEditMap(scope.row)">{{$t('plan.editMapping')}}</el-button>
          </template>
        </el-table-column>
        <el-table-column :label="$t('plan.delete')">
          <template slot-scope="scope">
            <el-button size="mini" plain :disabled="scope.$index === tableDataInit" @click="deleteData(scope.row)">{{$t('plan.delete')}}</el-button>
          </template>
        </el-table-column>
        <el-table-column :label="$t('plan.edit')">
          <template slot-scope="scope">
            ...
            <template v-else>
              <el-button size="mini" plain @click="edit(scope.row,scope.$index)">{{$t('plan.edit')}}</el-button>
            </template>
          </template>
        </el-table-column>
      </el-table>
