  ...
  <div class="document_generation_wrapper">
      ...
      <div class="tab">
        <el-row>
          <el-col :span="12" class="tab_col">
            <div class="tab_div" :class="{'active':navIndex == 1}" @click="changeIndex(1)">{{$t('documentGeneration.ImportSite')}}</div>
            <div class="tab_div" :class="{'active':navIndex == 2}" @click="changeIndex(2)">{{$t('documentGeneration.Watermarking')}}</div>
          </el-col>
          <el-col :span="12" class="btn_col">
            <el-button size="small" :disabled="tableDataInit!==-1" @click="addRecord">{{$t('documentGeneration.AddNewRecord')}}</el-button>
            <el-button size="small" @click="getListData">{{$t('Refresh')}}</el-button>
          </el-col>
        </el-row>
      </div>
    </div>
    <div class="import_site" v-if="navIndex == 1">
      <el-table border style="width: 100%" size="mini" :data="tableList">
        ...
        <el-table-column  :label="$t('documentGeneration.Active')"> 
          <template slot-scope="scope">
            <el-checkbox v-model="active" v-if="scope.$index === tableDataInit" :false-label="1" :true-label="0"></el-checkbox>
            <template v-else>
        ...
        <el-table-column  :label="$t('documentGeneration.IsSensitive')">
          <template slot-scope="scope">
            <el-checkbox v-model="isSensitive" v-if="scope.$index === tableDataInit" :false-label="1" :true-label="0"></el-checkbox>
            <template v-else>
        ...
        <el-table-column :label="$t('documentGeneration.AllowUpload')">
          <template slot-scope="scope">
            <el-checkbox v-model="allowUpload" v-if="scope.$index === tableDataInit" :false-label="1" :true-label="0"></el-checkbox>
            <template v-else>
        ...
        <el-table-column  :label="$t('documentGeneration.AllowGenerate')">
          <template slot-scope="scope">
            <el-checkbox v-model="allowGenerate" v-if="scope.$index === tableDataInit" :false-label="1" :true-label="0"></el-checkbox>
            <template v-else>
        ...
        <el-table-column :label="$t('documentGeneration.NeedWatermark')">
          <template slot-scope="scope">
            <el-checkbox v-model="needWatermark" v-if="scope.$index === tableDataInit" :false-label="1" :true-label="0"></el-checkbox>
            <template v-else>
        ...
        <el-table-column  :label="$t('update')" width="120">
          <template slot-scope="scope">
            <uploader :isDisabled="Boolean(scope.$index==tableDataInit&&scope.row.docId)" :fileId="'documentTable'+scope.row.docId" :maxSize="100" :uploadParam="uploadTableParam" @uploadAfter="uploadTableAfter" :url="$api.uploadDocument" fileType=".docx" :btnText="{import:$t('documentGeneration.SelectFile')}" :showType="1"></uploader>
          </template>
        </el-table-column>
        <el-table-column :label="$t('documentGeneration.Download')">
          <template slot-scope="scope">
            <el-button size="mini" plain :disabled="scope.$index === tableDataInit" @click="downDocument(scope.row)">{{$t('documentGeneration.Download')}}</el-button>
          </template>
        </el-table-column>
        <el-table-column :label="$t('Delete')">
          <template slot-scope="scope">
            <el-button size="mini" plain :disabled="scope.$index === tableDataInit" @click="deleteData(scope.row)">{{$t('Delete')}}</el-button>
          </template>
        </el-table-column>
    ...
    <div class="watermarking" v-if="navIndex == 2">
      <div class="prompt_text">
        <el-alert :title="$t('documentGeneration.waterImageCommand')" type="info" :closable="false">
        </el-alert>
      </div>
      <div class="upload_waterImg">
        <uploader fileId="waterFile" :maxSize="50" :uploadParam="uploadwaterParam" @uploadAfter="uploadwaterAfter" :url="$api.uploadWatermark" fileType=".png" :btnText="{select:$t('documentGeneration.SelectFile'),import:$t('update')}"></uploader>
        <el-button size="mini" @click="downWater">{{$t('documentGeneration.Download')}}</el-button>
        <el-button size="mini" @click="deleteWater">{{$t('documentGeneration.DeleteWatermarkImage')}}</el-button>
      </div>
    </div>
  </div>
</template>
