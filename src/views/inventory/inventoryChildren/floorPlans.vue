<template>
  <div class="floor_plan_wrapper">
    <div class="update_floor_plan floor_plan">
      <el-row class="row_header">
        <el-col :span="8" class="col_text">
          {{$t('floorPlans.updateFloorPlan')}}
        </el-col>
        <el-col :span="16" class="col_button">
          <uploader fileId="floorFile" :maxSize="10" :uploadParam="uploadParam" @uploadAfter="uploadExceAfter" :url="$api.importFloorPlan" fileType=".xls,.xlsx" :btnText="{select:$t('floorPlans.selectFile'),import:$t('floorPlans.importFloorPlan')}"></uploader>
          <el-button size="mini" @click="exportExcel"> {{$t('floorPlans.exportFloorPlantoExcel')}}
          </el-button>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="15">
          <el-alert :title="$t('floorPlans.updateFloorPlanCommand')" type="info" :closable="false">
          </el-alert>
        </el-col>
      </el-row>
    </div>
    <div class="floor_plan_image floor_plan">
      <el-row class="row_header">
        <el-col :span="8" class="col_text">
          {{$t('floorPlans.importFloorPlanImages')}}
        </el-col>
        <el-col :span="16" class="col_button">
          <uploader fileId="floorImage" :maxSize="50" :uploadParam="uploadParam" @uploadAfter="uploadImgAfter" :url="$api.importFloorPlanImg" fileType=".jpg,.png,.zip" :btnText="{select:$t('floorPlans.selectFile'),import:$t('floorPlans.addFloorPlanImages')}"></uploader>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="15">
          <el-alert :title="$t('floorPlans.importFloorPlanImagesCommand')" type="info" :closable="false">
          </el-alert>
        </el-col>
      </el-row>
    </div>
    <div class="floor_plan_list floor_plan">
      <el-row class="row_header">
        ...
        <el-col :span="12" class="col_button">
          <el-button size="mini" :disabled="tableDataInit!==-1" @click="add">{{$t('floorPlans.addFloorPlan')}}</el-button>
          <el-button size="mini" @click="downFloorPlan">{{$t('floorPlans.downFloorPlan')}}</el-button>
          <el-button size="mini" @click="refresh">{{$t('floorPlans.refresh')}}
          </el-button>
        </el-col>
      </el-row>
      <el-table :data="floorList" border style="width: 100%" :header-cell-style="{'background':'#f5f7fa'}" size="mini">
        ...
        <el-table-column :label="$t('floorPlans.3dModelUrl')">
          <template slot-scope="scope">
            <div v-if="scope.row.modelPath">{{hostUrl+scope.row.modelPath}}
        ...
        <el-table-column :label="$t('floorPlans.upload3dModel')">
          <template slot-scope="scope">
            <uploader v-if="scope.row.floorPlanId" :isDisabled="(scope.$index === tableDataInit&&!scope.row.modelPath)" :fileId="'floorModel'+scope.row.floorPlanId" :maxSize="300" :uploadParam="uploadModelParam" @uploadAfter="uploadModelAfter" :url="$api.uploadFloorPlan3D" fileType=".zip" :btnText="{import:$t('upLoad')}" :showType="1"></uploader>
            <template v-if="scope.row.modelPath">
              <el-button size="mini" plain @click="put3d(scope.row,'1','0')" v-if="scope.row.status3d==1">{{$t('floorPlans.PutOff')}}</el-button>
              <el-button size="mini" plain @click="put3d(scope.row,'1','1')" v-else>{{$t('floorPlans.PutOn')}}</el-button>
            </template>
          </template>
        </el-table-column>
        ...
        <el-table-column :label="$t('floorPlans.delete')">
          <template slot-scope="scope">
            <el-button size="mini" plain :disabled="scope.$index === tableDataInit" @click="deleteData(scope.row)">{{$t('floorPlans.delete')}}</el-button>
          </template>
        </el-table-column>
        <el-table-column :label="$t('floorPlans.edit')">
          <template slot-scope="scope">
            ...
            <template v-else>
              <el-button size="mini" plain @click="edit(scope.row,scope.$index)">{{$t('floorPlans.edit')}}</el-button>
            </template>
          </template>
        </el-table-column>
      </el-table>
