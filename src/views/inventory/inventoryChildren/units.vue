<template>
  <div class="units_wrapper">
    <div class="update_units units_section">
      <el-row class="row_header">
        ...
        <el-col :span="16" class="col_button">
          <uploader fileId="unit" :maxSize="100" :uploadParam="uploadUnitParam" @uploadAfter="uploadUnitsAfter" :url="$api.importUnit" fileType=".xls,.xlsx" :btnText="{select:$t('units.selectFile'),import:$t('units.importUnitList')}"></uploader>
          <el-button size="mini" @click="exportUnit"> {{$t('units.exportUnitList')}}
          </el-button>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="15">
          <el-alert :title="$t('units.updateUntilsCommand')" type="info" :closable="false">
          </el-alert>
        </el-col>
      </el-row>
    ...
    <div class="update_units_status units_section">
      <el-row class="row_header">
        <el-col :span="8" class="col_text">
          {{$t('units.updateUnitsStatus')}}
        </el-col>
        <el-col :span="16" class="col_button">
          <uploader fileId="unitTransaction" :maxSize="10" :uploadParam="uploadUnitTransactionParam" :url="$api.importUnitTransaction" fileType=".xls,.xlsx" :btnText="{select:$t('units.selectFile'),import:$t('units.importUnitTransactionList')}"></uploader>
          <el-button size="mini" @click="exportUnitTransaction">{{$t('units.exportUnitTransactionList')}}
          </el-button>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="15">
          <el-alert :title="$t('units.updateUnitsStatusCommand')" type="info" :closable="false">
          </el-alert>
        </el-col>
      </el-row>
    </div>
        ...
        <el-tabs v-model="activeTable" type="border-card">
          <el-tab-pane :label="$t('units.UnitList')" name="1">
            ...
            <el-table :data="unitList" :header-cell-style="{'background':'#f5f7fa'}" style="width: 100%" size="mini">
              <el-table-column prop="unitName" :label="$t('units.Name')">
              </el-table-column>
              <el-table-column prop="purchaseStatus" :label="$t('units.Status')">
              </el-table-column>
              <el-table-column prop="price1" :label="$t('units.Price1')">
              </el-table-column>
              <el-table-column prop="price2" :label="$t('units.Price2')">
              </el-table-column>
              <el-table-column prop="price3" :label="$t('units.Price3')">
              </el-table-column>
              <el-table-column prop="price4" :label="$t('units.Price4')">
              </el-table-column>
              <el-table-column prop="price5" :label="$t('units.Price5')">
              </el-table-column>
              <el-table-column prop="area" :label="$t('units.Area')">
              </el-table-column>
              <el-table-column prop="type" :label="$t('units.Type')">
              </el-table-column>
              <el-table-column :label="$t('Delete')">
                <template slot-scope="scope">
                  <el-button size="mini" @click="deleteUnit(scope.row)">{{$t('units.delete')}}</el-button>
                </template>
              </el-table-column>
              <el-table-column :label="$t('Edit')">
                <template slot-scope="scope">
                  <el-button size="mini" @click="editUnit(scope.row)">{{$t('units.edit')}}</el-button>
                </template>

              </el-table-column>
            </el-table>
            <div class="page_section">
              <el-pagination background small @size-change="handleUnitSizeChange" @current-change="handleUnitCurrentChange" :current-page.sync="unitPage.currentPage" :page-sizes="[5,10,30,50,100]" :page-size="unitPage.pageSize" layout="prev, pager, next,sizes,total" :total="unitPage.count">
              </el-pagination>
            </div>
          </el-tab-pane>
          <el-tab-pane :label="$t('units.UnitAccessList')" name="2">
            <div class="unit_list_header">
              <el-row>
                ...
                <el-col :span="12">
                  <el-button size="mini" :disabled="groupInit!==-1" @click="addGroup">{{$t('units.AddUnitAccess')}}</el-button>
                  ...
                  <div class="tbody_item flex" v-for="(item,k) in unitAccessList">
                    ...
                    <div class="tbody_item_right">
                      <div class="right_item flex opration_column">
                        ...
                        <div class="opration_column_children">
                          <el-button size="mini" @click="manageUnits(item)" :disabled="!item.group.groupId">{{$t('units.ManageUnits')}}({{item.group.unitNum}})</el-button>
                        </div>
                        <div class="opration_column_children">
                          <el-button size="mini" :disabled="groupInit==k" @click="deleteGroup(item)">{{$t('units.delete')}}</el-button>
                        </div>
                        <div class="opration_column_children">
                          <template v-if="groupInit!==k">
                            <el-button size="mini" @click="editGroup(item,k)">{{$t('units.edit')}}</el-button>
                          </template>
                          <template v-else>
                            <el-button size="mini" @click="updateGroup(item)">{{$t('update')}}</el-button>
                            <el-button size="mini" @click="cancelEditGroup(item,k)">{{$t('cancel')}}</el-button>
                          </template>
                        </div>
                      </div>
                      <div class="right_item flex add_column">
                        <div class="add ">
                          <el-button size="mini" icon="el-icon-plus" @click="addTeam(item,k)" :disabled="!item.group.isAddTeam">{{$t('units.AddTeam')}}</el-button>
                        </div>
                      </div>
                        ...
                        <div class="data_column_item flex" v-for="(team,i) in item.teams">
                          <div class="data_column_item_left flex">
                            ...
                            <div v-if="!team.teamId">
                              <el-button size="mini" @click="insertTeamToGroup(item)">{{$t('units.Insert')}}</el-button>
                              <el-button size="mini" @click="cancelTeam(team,i,item)">{{$t('cancel')}}</el-button>
                            </div>
                          </div>
                          <div class="data_column_item_right">
                            <el-button size="mini" @click="viewTeamMembers(item,team)" :disabled="!team.teamId&&!item.group.selectTeamId">{{$t('units.ViewTeamMembers')}}</el-button>
      ...
      <div class="edit_price_content">
        <div class="header">
          <el-row>
            ...
            <el-col :span="12">
              <div class="opration">
                <el-button size="mini" @click="updateUnit">{{$t('update')}}</el-button>
                <el-button size="mini" @click="dialogOfPrice = false">{{$t('cancel')}}</el-button>
              </div>
            </el-col>
          </el-row>
        </div>
    ...
    <el-dialog :title="$t('units.ManageUnitsUnderGroup')" :visible.sync="dialogOfGroup" width="900px" @close="closeGroupDialog">
      <div class="manage_group_content">
        <el-row>
          <el-col>
            <div class="header">
              <uploader ref="unitManageImport" fileId="unitManageImport" :maxSize="10" :uploadParam="importUnitGroupParam" :url="$api.importUnitGroup" fileType=".xls,.xlsx" :btnText="{select:$t('units.selectFile'),import:$t('units.Import')}" @uploadAfter="uploadManageUnitAfter"></uploader>
              <el-button size="mini" @click="exportUnitManage">{{$t('units.Export')}}</el-button>
              <el-button size="mini" @click="deleteGroupOfUnit('')">{{$t('units.RemoveAll')}}</el-button>
              <el-button size="mini" @click="refreshManageUnit">{{$t('Refresh')}}</el-button>
            </div>
          </el-col>
        </el-row>
        <el-row class="select_section_row">
          ...
          <el-col :span="4">
            <div class="select_btn">
              <el-button size="mini" @click="insertUnitGroupJoin">{{$t('units.Insert')}}</el-button>
            </div>
          </el-col>
        </el-row>
        <div class="table_section">
          <el-table :data="unitsManageList" border :header-cell-style="{'background':'#f5f7fa'}" style="width: 100%" max-height='300' size="mini">
            <el-table-column prop="unitName" :label="$t('units.UnitName')">
            </el-table-column>
            <el-table-column :label="$t('Delete')" width="180">
              <template slot-scope="scope">
                <el-button size="mini" @click="deleteGroupOfUnit(scope.row.unitId)">{{$t('units.delete')}}</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
        <div class="page_section" v-if="unitsManageList.length">
          <el-pagination background small @size-change="handleUnitManageSizeChange" @current-change="handleUnitManageCurrentChange" :current-page.sync="unitManagePage.currentPage" :page-sizes="[5,10,30,50,100]" :page-size="unitManagePage.pageSize" layout="prev, pager, next,sizes,total" :total="unitManagePage.count">
          </el-pagination>
        </div>
      </div>
    </el-dialog>
