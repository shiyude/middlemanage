    ...
    <div class="head">
      <el-row>
        ...
        <el-col :span="12" style="text-align:right">
          <el-button size="mini" @click="add" :disabled="tableDataInit!==-1">{{$t('buildingPhases.addBuilding')}}</el-button>
    ...
    <div class="building_content">
      <el-table :data="buildingList" border style="width: 100%" :header-cell-style="{'background':'#f5f7fa'}" size="mini">
        ...
        <el-table-column :label="$t('buildingPhases.delete')">
          <template slot-scope="scope">
            <el-button size="mini" plain :disabled="scope.$index === tableDataInit" @click="deleteData(scope.row)">{{$t('buildingPhases.delete')}}</el-button>
          </template>
        </el-table-column>
        <el-table-column :label="$t('buildingPhases.edit')">
          <template slot-scope="scope">
            ...
            <template v-else>
              <el-button size="mini" plain @click="edit(scope.row,scope.$index)">{{$t('buildingPhases.edit')}}</el-button>
            </template>
          </template>
        </el-table-column>
      </el-table>
