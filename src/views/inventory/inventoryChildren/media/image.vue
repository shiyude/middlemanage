<template>
  <div class="images_wrapper">
    <div class="import_image image_section">
      ...
      <el-row>
        <el-col :span="15">
          <el-alert :title="$t('image.importImagesCommand')" type="info" :closable="false">
          </el-alert>
        </el-col>
      </el-row>
    </div>
    <div class="add_main_image image_section">
      <el-row class="row_header">
        <el-col :span="8" class="col_text">
          {{$t('image.addtheMainImage')}}
        </el-col>
        <el-col :span="16" class="col_button">
          <!-- <el-button size="mini"> {{$t('image.selectFile')}}</el-button>
          <el-button size="mini"> {{$t('image.addMainImage')}}
          </el-button> -->
          <uploader fileId="mainImageFile" :maxSize="10" :uploadParam="uploadMainParam" :url="$api.uploadProjectMainImg" @uploadAfter="uploadMainImageAfter" fileType=".jpg,.jpeg,.png,.gif" :btnText="{select:$t('image.selectFile'),import:$t('image.addMainImage')}"></uploader>
      ...
      <el-row>
        <el-col :span="15">
          <el-alert :title="$t('image.addtheMainImageCommand')" type="info" :closable="false">
          </el-alert>
        </el-col>
      </el-row>
    </div>
    <div class="images_lists image_section">
      <el-row class="row_header">
        ...
        <el-col :span="12" class="col_button">
          <el-button size="mini" @click="add" :disabled="tableDataInit!==-1">{{$t('image.addImageWithUrl')}}
          </el-button>
          <el-button size="mini" @click="refresh">{{$t('image.refresh')}}
          </el-button>
        </el-col>
      </el-row>
      <el-table :data="imageList" border style="width: 100%" :header-cell-style="{'background':'#f5f7fa'}" size="mini">
        ...
        <el-table-column :label="$t('image.delete')">
          <template slot-scope="scope">
            <el-button size="mini" plain :disabled="scope.$index === tableDataInit" @click="deleteData(scope.row)">{{$t('image.delete')}}</el-button>
          </template>
        </el-table-column>
        <el-table-column :label="$t('image.edit')">
          <template slot-scope="scope">
            ...
            <template v-else>
              <el-button size="mini" plain @click="edit(scope.row,scope.$index)">{{$t('image.edit')}}</el-button>
            </template>
          </template>
        </el-table-column>
      </el-table>
      <div class="page_section" v-if="imageNum">
        <el-pagination background small @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page.sync="currentPage" :page-sizes="[5,10,30,50,100]" :page-size="pageSize" layout="prev, pager, next,sizes,total" :total="imageNum">
        </el-pagination>
      </div>
    </div>
  </div>
