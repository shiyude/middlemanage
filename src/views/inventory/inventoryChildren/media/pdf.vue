<template>
  <div class="pdfs_wrapper">
    <div class="import_pdf pdf_section">
      ...
      <el-row>
        <el-col :span="15">
          <el-alert :title="$t('pdf.importPDFsCommand')" type="info" :closable="false">
          </el-alert>
        </el-col>
      </el-row>
    </div>
    <div class="pdf_lists pdf_section">
      <el-row class="row_header">
        ...
        <el-col :span="12" class="col_button">
          <el-button size="mini" @click="add" :disabled="tableDataInit!==-1">{{$t('pdf.addPDFWithUrl')}}
          </el-button>
          <el-button size="mini" @click="refresh">{{$t('pdf.refresh')}}
          </el-button>
        </el-col>
      </el-row>
      <el-table :data="pdfList" border style="width: 100%" :header-cell-style="{'background':'#f5f7fa'}" size="mini">
        ...
        <el-table-column :label="$t('pdf.delete')">
          <template slot-scope="scope">
            <el-button size="mini" plain :disabled="scope.$index === tableDataInit" @click="deleteData(scope.row)">{{$t('pdf.delete')}}</el-button>
          </template>
        </el-table-column>
        <el-table-column :label="$t('pdf.edit')">
          <template slot-scope="scope">
            ...
            <template v-else>
              <el-button size="mini" plain @click="edit(scope.row,scope.$index)">{{$t('pdf.edit')}}</el-button>
            </template>
          </template>
        </el-table-column>
      </el-table>
